...
结果先显示在弹窗中，可直接调整时间、清单、标签、优先级和预估，区分 AI 建议与人工修改，并能恢复原建议。只保存最终勾选且确认的草稿；取消不会修改任务。超过 40 项时按每批最多 40 项依次请求，全部成功后显示结果；每批消耗接口配额。应用前检查本地与云端状态，已编辑、改期、完成或删除的任务会跳过，并报告应用、跳过与失败数量。
...
先按顺序应用 [supabase/migrations/](./supabase/migrations) 中的迁移（目前为 `0001`–`0007`）。`0006` 包含通知归属与清空标记，`0007` 新增重复任务与事务去重。未部署 `0007` 时重复设置会禁用。数据库部署约定见 [Supabase 说明](./supabase/README.md)。
...
`pnpm check` 会运行 ESLint、路由类型生成与 TypeScript、国际化、认证、持久化、同步恢复、审查回归及 Prettier 检查。新增回归覆盖 AI 分批/冲突、上海提醒时间、专注检查点、统计异常与畸形 WAV；部分界面分支使用静态检查。持久化测试使用数据库替身，不替代真实 Supabase、邮箱或麦克风验收。
...
- 音效、通知与自动休息保存为账户偏好；自动下一轮仅作用于当前计时。完整离线编辑与跨设备同步仍待 SYNC-01，不把浏览器日志视为完整离线能力。
