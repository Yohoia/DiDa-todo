...
先按顺序应用 [supabase/migrations/](./supabase/migrations) 中的迁移（目前为 `0001`–`0013`）。`0006` 包含通知归属与清空标记，`0007` 新增重复任务与事务去重，`0008` 启用工作台 Realtime，`0009` 新增服务端任务搜索，`0010` / `0011` 分别新增统计成长通知与账号时区、审计能力，`0012` 修复聚合通知的 RLS 策略。`0013` 新增任务/子任务的原子版本更新及真实逐日完成数据；需由 GitHub Actions 执行新增迁移。数据库部署约定见 [Supabase 说明](./supabase/README.md)。
