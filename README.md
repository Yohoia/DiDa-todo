...
先按顺序应用 [supabase/migrations/](./supabase/migrations) 中的迁移（目前为 `0001`–`0011`）。`0006` 包含通知归属与清空标记，`0007` 新增重复任务与事务去重，`0008` 启用工作台 Realtime，`0009` 新增服务端任务搜索，`0010` / `0011` 分别新增统计成长通知与账号时区、审计能力。目标 Supabase 已部署到 `0009`；`0010` / `0011` 待推送后的迁移工作流执行。数据库部署约定见 [Supabase 说明](./supabase/README.md)。
...
工作区布局在服务端预取当前账号的未完成任务与账户时区今天完成项，`WorkspaceProvider` 维护客户端状态与乐观更新，再通过 Repository 串行同步到 Supabase；旧完成历史由完成页分页加载，历史日期按选中日期补拉，搜索走服务端分页 RPC。Realtime 私有订阅按账号过滤任务、子任务、偏好与通知，事件触发快照回读并等待本地待写上传，同时补充重取已加载的历史完成项，顶部显示连接状态。个人统计在数据库侧聚合真实的任务与专注记录，成长奖励使用幂等事件表，避免查询行数上限或重试造成漏算与重复奖励。
