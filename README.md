...
先按顺序应用 [supabase/migrations/](./supabase/migrations) 中的迁移（目前为 `0001`–`0012`）。`0006` 包含通知归属与清空标记，`0007` 新增重复任务与事务去重，`0008` 启用工作台 Realtime，`0009` 新增服务端任务搜索，`0010` / `0011` 分别新增统计成长通知与账号时区、审计能力，`0012` 修复聚合通知的 RLS 策略。目标 Supabase 已部署到 `0011`；`0012` 由下一次推送 `main` 后的迁移工作流执行。数据库部署约定见 [Supabase 说明](./supabase/README.md)。
...
`pnpm check` 会运行 ESLint、路由类型生成与 TypeScript、国际化、认证、持久化、同步恢复、Realtime、审查回归、阶段一至三、语音回归及 Prettier 检查。新增回归覆盖 AI 分批/冲突、账户时区、专注检查点、统计月界、成长奖励幂等、账号删除/导出边界、通知改期去重、语音错误矩阵与畸形 WAV；部分界面分支使用静态检查。持久化与 Realtime 测试使用数据库替身或本地迁移验证，不替代真实 Supabase、邮箱或麦克风验收。
