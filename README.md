...
<p align="center">
  ...
  <br>
  <sub>邮箱认证 · 云端任务同步 · AI 整理与顾问 · 专注计时 · 站内提醒</sub>
</p>
...
| 回顾成长   | 已完成任务、专注热力图、目标统计、等级与永久花园 | `/completed`、`/insight`、`/profile` |
| 管理账户   | 资料与认证、数据导出、删除账号、语音历史与热词   | `/settings`                          |
| 接收提醒   | 默认提醒、每日摘要、保留期、已读与清空           | 工作区底部 Dock 的通知入口           |
...
整理草稿复用完整任务详情表单，日期显示但不能改期。One Thing、删除及开始专注是即时操作，整理中显示但禁用，请先应用后在普通详情执行。重复设置遵循数据库迁移能力开关；包含子任务的保存通过数据库事务校验任务版本并同时更新，失败整笔回滚。
...
- 新任务可按账户默认提前量设置提醒；每日摘要在账户时区早上 8 点后生成一次。
- 点击通知打开任务时标记已读，「全部已读」只消除未读标记；超过保留期的通知会物理清理。
- 「清空通知」需要二次确认，隐藏现有已读与未读通知，不删除待办。
- 清空状态保存到云端，同时保留提醒实例去重记录；任务改期后可生成下一次提醒，旧提醒不会因刷新或补扫重生。
- 删除任务时，其对应通知随之删除。
- 不申请系统通知权限，也不在关闭页面后后台推送或发送邮件。
...
先按顺序应用 [supabase/migrations/](./supabase/migrations) 中的迁移（目前为 `0001`–`0016`）。`0006` 包含通知归属与清空标记，`0007` 新增重复任务与事务去重，`0008` 启用工作台 Realtime，`0009` 新增服务端任务搜索，`0010` / `0011` 分别新增统计成长通知与账号时区、审计能力，`0012` 修复聚合通知的 RLS 策略，`0013` 新增任务/子任务的原子版本更新及真实逐日完成数据，`0014` / `0015` 分别修复账号级联删除与统计账户时区，`0016` 为通知补充语言无关展示数据；需由 GitHub Actions 执行新增迁移。数据库部署约定见 [Supabase 说明](./supabase/README.md)。
...
设置页可导出本人业务数据，导出不包含密码、令牌或其他凭据；删除账号必须输入当前邮箱确认，服务端以特权操作删除 Auth 用户并级联清理业务数据，仅保留不含业务内容的审计记录。语音隐私区可查看、单删、清空最近转写，并维护转写热词；原始音频不落库。

每个账户可保存 IANA 时区与 12/24 小时制，默认上海且不自动跟随设备。任务、日历、提醒、语音相对日期、重复任务与统计使用同一账户时区；切换偏好不会重写任务事实。
...
Insights 的 AI 顾问复用上述服务端配置，用于子任务拆分、独立预估、容量判断与周回顾。建议先预览、编辑、取消或勾选，应用时只写入勾选任务的预估与新增子任务，不修改标题、日期或时间。
...
未配置 Key 时其余手动功能不受影响，语音入口会提示服务未配置。应用不将音频落盘或写入数据库；音频会发送至转写供应商，转写文本会发送至解析 LLM。用户确认添加后，转写文本与解析结果保存到本人账户的 `voice_captures` 表，可在设置页查看、单删或清空，转写请求会使用本人热词。
...
工作区布局在服务端预取当前账号的未完成任务与账户时区今天完成项，`WorkspaceProvider` 维护客户端状态与乐观更新，再通过 Repository 串行同步到 Supabase；旧完成历史由完成页分页加载，历史日期按选中日期补拉，搜索走服务端分页 RPC。Realtime 使用账号过滤且受表级 RLS 保护的唯一 channel，监听任务、子任务、偏好与通知，事件触发快照回读并等待本地待写上传，同时补充重取已加载的历史完成项，顶部显示连接状态。个人统计在数据库侧聚合真实的任务与专注记录，成长奖励使用幂等事件表，避免查询行数上限或重试造成漏算与重复奖励。
...
`pnpm check` 会运行 ESLint、路由类型生成与 TypeScript、国际化、认证、持久化、同步恢复、Realtime、审查回归、阶段一至三、语音回归及 Prettier 检查。新增回归覆盖 AI 分批/冲突、账户时区、专注检查点、统计月界、成长奖励幂等、账号删除/导出边界、通知改期去重、语音错误矩阵与畸形 WAV；部分界面分支使用静态检查。持久化与 Realtime 测试使用数据库替身或本地迁移验证；目标 Supabase 已另做重复任务、双端同步、断线重连、并发冲突与搜索分页验收，但不替代邮箱或麦克风验收。
...
- 音效、通知与自动休息保存为账户偏好；自动下一轮仅作用于当前计时。Realtime、历史按需读取与服务端搜索已实现，`0008` / `0009` 目标库迁移、双端同步、断线重连、并发冲突与 33 条搜索分页验收已通过；创建/删除等完整离线编辑仍待 SYNC-01，不把浏览器日志视为完整离线能力。
- 站内通知没有后台推送或系统通知；保留期会物理删除旧通知，清空是持久隐藏并保留提醒实例去重记录。
- 统计、经验值和永久花园从本人真实任务与专注记录聚合；首页宣传内容与静态展示资源不代表实时业务数据。
