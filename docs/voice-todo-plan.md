...
> 2026-09-17：M1/M2 是历史实施记录；M3 真机录音、VAD 与语料回归待完成，统一见 [VOICE-01 / QA-01](implementation-plan.md)。无日期语音任务已有「未安排」入口，WAV 截断校验已补强。下方供应商实测记录保留原日期，不代表本次重新付费联调。

> TASK-03 更新：确认卡勾选状态与胶囊快捷确认统一；单条/多条均可「逐项调整」，复用文字录入的可编辑确认列表。保存中锁定两处操作，成功项移出、失败项稳定 ID 重试，语音历史只记成功事项。历史的“多条隐藏编辑”描述不再适用于当前版本，详见 [阶段交付记录](stage-delivery-20260917.md)。
...
VOICE_ASR_BASE_URL = （可选）ASR 根域名，默认 https://ws-9mlt2qkeiwpfmr0z.ap-southeast-1.maas.aliyuncs.com（新加坡工作空间，原生 multimodal-generation 协议）
