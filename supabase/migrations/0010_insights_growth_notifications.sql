-- Stage three foundations: richer insight facts, permanent growth rewards,
-- and notification preferences/deduplication keys.

alter table public.user_preferences
  add column if not exists daily_focus_goal_minutes int
    default 120 check (daily_focus_goal_minutes between 15 and 960);
alter table public.user_preferences
  add column if not exists default_reminder_minutes int
    default 0 check (default_reminder_minutes between 0 and 120);
alter table public.user_preferences
  add column if not exists notification_retention_days int
    default 7 check (notification_retention_days between 1 and 90);
alter table public.user_preferences
...
-- A notification can now be tied to a task or carry a stable aggregate key.
alter table public.notifications add column if not exists dedupe_key text;
alter table public.notifications
  drop constraint if exists notifications_type_check;
alter table public.notifications
  add constraint notifications_type_check
  check (type in ('task_due', 'daily_digest'));
create unique index if not exists notifications_user_dedupe_key
  on public.notifications (user_id, dedupe_key)
  where dedupe_key is not null;
drop index if exists notifications_task_once;
create index if not exists notifications_user_task_created
  on public.notifications (user_id, task_id, created_at desc);

-- Insight aggregation remains based on immutable task/focus facts. Estimated
-- effort uses the owner's pomodoro duration at query time; actual effort uses
-- focus sessions linked to completed tasks in the Shanghai calendar month.
...
as $$
  ...
  completed as (
    ...
    where user_id = auth.uid()
      ...
      and (completed_at at time zone 'Asia/Shanghai')::date <= boundaries.today
  ),
  completed_effort as (
    select
      completed.id,
  ...
  select jsonb_build_object(
    ...
    'unlinkedTaskSecondsThisMonth', coalesce((
      ...
      where focus_sessions.user_id = auth.uid()
        ...
        and (focus_sessions.started_at at time zone 'Asia/Shanghai')::date
          >= date_trunc('month', boundaries.today)::date
        and (focus_sessions.started_at at time zone 'Asia/Shanghai')::date
          <= boundaries.today
    ), 0),
...
-- Permanent rewards have an idempotency key. Retrying a checkpoint or profile
-- load can never mint the same plant, coin, or achievement twice.
create table if not exists public.growth_events (
  user_id uuid not null references auth.users (id) on delete cascade,
  event_type text not null check (event_type in ('focus_plant', 'achievement')),
...
create unique index if not exists growth_events_user_key
  on public.growth_events (user_id, event_key);

create table if not exists public.growth_plants (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
  event_key text not null,
  symbol text not null check (symbol in ('tree', 'forest', 'potted', 'leaf')),
  planted_at timestamptz not null,
  created_at timestamptz not null default now(),
  foreign key (user_id, event_key)
    references public.growth_events (user_id, event_key) on delete cascade
);
create index if not exists growth_plants_user_time
  on public.growth_plants (user_id, planted_at desc);
create unique index if not exists growth_plants_user_event_key
  on public.growth_plants (user_id, event_key);

alter table public.growth_events enable row level security;
alter table public.growth_plants enable row level security;

drop policy if exists "growth events select own" on public.growth_events;
create policy "growth events select own" on public.growth_events
  for select using (auth.uid() = user_id);
drop policy if exists "growth plants select own" on public.growth_plants;
create policy "growth plants select own" on public.growth_plants
  for select using (auth.uid() = user_id);

grant select on public.growth_events, public.growth_plants to authenticated;
...
begin
  ...
  select gamification_enabled into enabled
  from public.user_preferences where user_id = current_user_id;
  if not coalesce(enabled, true) then return; end if;

  -- Every completed 120-minute block is one permanent plant. The boundary is
  -- computed from ordered immutable sessions, and the insert is idempotent.
  with ordered_focus as (
    select
      duration_seconds,
      ended_at,
      sum(greatest(duration_seconds, 0)) over (order by started_at, id) as cumulative_seconds,
  ...
  select max(streak) into max_streak from (
    ...
    from (
      ...
      from (
        select distinct ((started_at at time zone 'Asia/Shanghai')::date) as day
        from public.focus_sessions
        where user_id = current_user_id and mode = 'focus'
      ) focus_days
    ) numbered
